...
| SMS/iMessage reply from Mac notification | Partial | macOS text reply sends `message.reply`; Android executes notification `RemoteInput` or SMS/default-role route | iMessage relay unavailable; SMS direct send requires role/policy |
...
Release-ready Plink means the Android and macOS apps are native, buildable, tested, locally pairable, and honest about permission-gated behavior. Private Apple service parity is not claimed.
