...
internal suspend fun checkFileRoundtrip(context: Context, arguments: Bundle, progress: (String) -> Unit): String = coroutineScope {
    ...
    val coordinator = FileTransferCoordinator(stagingBase = File(root, "stage"), scope = this,
        ...
        sendEnvelope = { envelope, _, stillValid -> check(stillValid()); client.send(envelope) }, scheduleWatchdog = false)
    try {
        server.start()
        ...
        withTimeout(900_000) {
            for (size in listOf(0, 1, 32_768, 32_769, 16_777_216)) {
                ...
                while (true) {
                    coordinator.handle(server.receiveOnce(), generation = 91)
                ...
                progress("FILE ROUNDTRIP boundary=$size: Android returned bytes verified and temporary files removed")
            }
        }
